...
SimpleImplicant[] systemic(uint[] F, uint[] R, char[] column_names)
{
    ...
    while (true)
    {
        if (state.SHOW_PROGRESS)
        {
            ...
            {
                cwritefln("it %s | paths <lred>%s</lred> capped | implicants remaining to check %s", i, paths.length, simple_implicants
                        .length - i);
            }
            else
            {
                writefln("it %s | paths %s | implicants remaining to check %s", i, paths.length, simple_implicants
                        .length - i);
            }
        }
